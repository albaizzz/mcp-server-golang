...
async function callLlama3Generic(req) {
    ...
    if (openaiCompat && openaiCompat.ok) {
        const j = await openaiCompat.json();
        if (j.choices?.[0]?.text)
            return { text: j.choices[0].text };
    ...
    if (tgi && tgi.ok) {
        const j = await tgi.json();
        if (j.generated_text)
            return { text: j.generated_text };
        if (j.output && Array.isArray(j.output)) {
            const blocks = j.output[0]?.content;
            if (Array.isArray(blocks))
                return { text: blocks.map((b) => b.text ?? b).join("") };
    ...
    if (raw && raw.ok) {
        ...
        if (j.generated_text)
            return { text: j.generated_text };
        if (j.data?.[0]?.generated_text)
            return { text: j.data[0].generated_text };
